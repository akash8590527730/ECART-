     ...
     <div className='grid grid-cols-3 gap-4 mt-5'>
      ...
      <tbody>
       { userCart?.map((product,index)=>(
         ...
         <td>{index+1}</td>
         <td>{product?.title}</td>
         <td><img width={'70px'}  height={'70px'} src={product?.thumbnail} alt="no img" /></td>
         <td>
           <div className='flex'>
             <button onClick={()=>handleDecrementQuantity(product)} className='font-bold'>-</button>
             <input style={{width:'40px'}} value={product?.quantity} className='border p-1 rounded ms-2 me-2' type="text" readOnly />
